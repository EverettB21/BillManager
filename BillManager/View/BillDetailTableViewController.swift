...
class BillDetailTableViewController: UITableViewController, UITextFieldDelegate {
    ...
    func presentNeedAuth() {
        let ac = UIAlertController(title: "Need Authorization", message: "Please allow notifications in settings", preferredStyle: .alert)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if remindSwitch.isOn {
            bill.setReminder(remindDatePicker.date) { (updated) in
                if let _ = updated.notificationID {
                    Database.shared.updateAndSave(updated)
                } else {
                    self.presentNeedAuth()
                }
            }
        } else {
            bill.removeReminder()
